...
def train_model(config):
    ...
    log_dir = os.path.join(config['logging']['log_dir'], f'ASTGCN_new_graph_{current_time}')
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs_batch, targets_batch) in enumerate(train_loader):
            ...
            # Forward pass
            outputs = model(inputs_batch)  # Shape: (batch_size, num_nodes, num_for_predict)

            # Extract outputs for the target nodes (6-11)
            outputs_target_nodes = outputs[:, 6:12, :].squeeze(-1)  # Shape: (batch_size, num_target_nodes)

            # Compute loss for all target nodes
            loss = criterion(outputs_target_nodes, targets_batch.view(-1, 6))  # Reshape target batch to match
        ...
        with torch.no_grad():
            for inputs_batch, targets_batch in val_loader:
                ...
                # Extract outputs for the target nodes (6-11)
                outputs_target_nodes = outputs[:, 6:12, :].squeeze(-1)

                # Compute loss for all target nodes
                loss = criterion(outputs_target_nodes, targets_batch.view(-1, 6))
        ...
        if val_loss < best_val_loss:
            ...
            model_name = f"astgcn_new_graph_best_{current_time}.pth"
