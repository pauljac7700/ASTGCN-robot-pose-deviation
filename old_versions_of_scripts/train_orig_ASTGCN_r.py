...
def train_model(config):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs_batch, targets_batch) in enumerate(train_loader):
            ...
            outputs_x_dif = outputs[:, 7, :]  # Shape: (batch_size, num_for_predict)

            # Since num_for_predict is 1, squeeze the last dimension
            outputs_x_dif = outputs_x_dif.squeeze(-1)  # Shape: (batch_size,)

            # Flatten targets
            targets_flat = targets_batch.view(-1)  # Shape: (batch_size,)

            # Compute loss
            loss = criterion(outputs_x_dif, targets_flat)
