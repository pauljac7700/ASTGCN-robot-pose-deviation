...
def prepare_data(config):
    ...
    # Then split train and validation sets with shuffle on False
    ...
    def create_sequences(inputs_df, target_df):
        ...
        num_nodes = config['model']['num_of_vertices']  # e.g., 8
        in_channels = config['model']['in_channels']  # e.g., 6
        ...
        for i in range(total_samples):
            ...
            for t in range(len_input):
                ...
                joint_positions = inputs_raw[idx, joint_indices]
                # Node 6: End-effector setpoints
                end_effector_setpoints = inputs_raw[idx, setpoint_indices]

                # Node 7: Error node (`x_dif`)
                # Exclude `x_dif` at time t if t == len_input - 1 (current time step)
                if t < len_input - 1:
                    x_dif_value = inputs_raw[idx, x_dif_index]
                else:
                    x_dif_value = 0  # Or set to a placeholder value or remove this feature
                ...
                # Assign `x_dif` for past time steps only
                if t < len_input - 1:
                    input_sequence[t, 7, 0] = x_dif_value
                else:
                    input_sequence[t, 7, 0] = 0  # Exclude current `x_dif` from inputs

            # Transpose to (num_nodes, in_channels, len_input)
            input_sequence = input_sequence.transpose(1, 2, 0)
            inputs.append(input_sequence)

            # Target value (`x_dif` at current time step)
            target_idx = i + len_input - 1
