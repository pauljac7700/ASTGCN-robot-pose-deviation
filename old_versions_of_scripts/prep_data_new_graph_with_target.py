...
def prepare_data(data, target_columns, val_size=0.15, test_size=0.20, random_seed=42):
    # Since target variables are part of the inputs, include all columns in features
    features = data.copy()
...
# Apply scaling and split data
def apply_scaling(X_train, X_val, X_test, y_train, y_val, y_test):
    # Initialize scalers dictionary
    scalers = {}

    # Fit scalers only on the training set for features (including target variables)
    ...
    # Scale the targets using the same scalers
    for target_col in y_train.columns:
        scaler = scalers[target_col]
        y_train[target_col] = scaler.transform(y_train[[target_col]])
