...
def prepare_data(config):
    ...
    # Include deviation variables in input features if desired
    # For now, let's include them as input features
    input_features = joint_features + setpoint_features + target_variables
    ...
    def create_sequences(inputs_df, target_df):
        ...
        dif_indices = [inputs_df.columns.get_loc(f) for f in target_variables]
        ...
        for i in range(total_samples):
            ...
            for t in range(len_input + 1):  # Includes time t + 1
                ...
                if t < len_input:
                    ...
                    dif_values = np.zeros(len(target_variables))  # At time t + 1, set deviations to zero
                ...
                # Node 7: Error node (deviations)
                input_sequence[t, 7, 0:len(target_variables)] = dif_values  # Assign deviation features
