...
def prepare_data(config):
    ...
    # Define input features and residual variable
    residual_variable = config['single_residual_variable']  # The residual variable is configurable and specified in the config file
    ...
    if prep_data_incl_past_residuals == 'nr':
        ...
        input_features = joint_features + target_pose_features + [residual_variable]

    # Ensure all required columns are present
    required_columns = input_features + [residual_variable]
    ...
    residual_df = df[[residual_variable]]
    ...
    # Scale the pose residual variable
    residual_scaler = StandardScaler()
    residual_scaler.fit(residual_train_df[[residual_variable]].values)
    residual_train_df[residual_variable] = residual_scaler.transform(residual_train_df[[residual_variable]].values)
    residual_val_df[residual_variable] = residual_scaler.transform(residual_val_df[[residual_variable]].values)
    residual_test_df[residual_variable] = residual_scaler.transform(residual_test_df[[residual_variable]].values)

    # Save the scalers
    scalers = {'input_scalers': input_scalers, 'residual_scalers': residual_scaler}
    ...
    def create_sequences(inputs_df, residual_df, prep_data_incl_past_residuals):
        inputs_raw = inputs_df.values
        residuals_raw = residual_df.values
        ...
        if prep_data_incl_past_residuals == 'wr':
            residual_variable_index = inputs_df.columns.get_loc(residual_variable)  # Dynamically get index of target variable
        else:
            # Removed dif_indices since residual variables are not in inputs_df
            pass
        ...
        for i in range(total_samples):
            ...
            for t in range(len_input + 1):  # Includes time t + 1
                ...
                end_effector_setpoints = inputs_raw[idx, target_pose_indices]  # Shape: (6,)
                # Node 7: Error node (residual variable from config)
                if prep_data_incl_past_residuals == 'wr':
                    if t < len_input:
                        dif_value = inputs_raw[idx, residual_variable_index]  # Use past residual variable values
                    else:
                        dif_value = 0  # At time t + 1, set deviations to zero
                else:
                    dif_value = np.zeros(len(residual_variable))  # Since we're excluding deviations from inputs
                ...
                input_sequence[t, num_joints + 1, 0:len(residual_variable)] = dif_value # Assign deviation features
            ...
            residual_value = residuals_raw[residual_idx]
            residuals.append(residual_value[0])

        inputs = np.array(inputs)  # Shape: (num_samples, num_nodes=8, in_channels=6, len_input + 1)
        residuals = np.array(residuals)  # Shape: (num_samples,)

        return inputs, residuals

    # Create sequences for training, validation, and test sets
    inputs_train, residuals_train = create_sequences(inputs_train_df.reset_index(drop=True), residual_train_df.reset_index(drop=True), prep_data_incl_past_residuals)
    inputs_val, residuals_val = create_sequences(inputs_val_df.reset_index(drop=True), residual_val_df.reset_index(drop=True), prep_data_incl_past_residuals)
    inputs_test, residuals_test = create_sequences(inputs_test_df.reset_index(drop=True), residual_test_df.reset_index(drop=True), prep_data_incl_past_residuals)
