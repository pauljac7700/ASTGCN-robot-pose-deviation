...
def build_adjacency_matrix_5(num_joints: int) -> np.ndarray:
    """
    ...
    - Each joint node is **additionally connected to both target pose nodes**.
    ...
    # Connect each joint node to **both target pose nodes**
    for joint_node in range(num_joints):
        adjacency_matrix[joint_node, target_position_node] = 1
        adjacency_matrix[target_position_node, joint_node] = 1
        adjacency_matrix[joint_node, target_rotation_node] = 1
        adjacency_matrix[target_rotation_node, joint_node] = 1
...
if __name__ == "__main__":
    ...
    adj_matrix = build_adjacency_matrix_5(num_joints)

    filename = "data/adjacency_matrix_5_6D.npy"
